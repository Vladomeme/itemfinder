    ...
    public void onInitialize() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> dispatcher.register(
                literal("finditem")
                        ...
                        .then(literal("empty")
                                .executes(context -> LootTableFinder.search("none_empty", context))
                                .then(literal("global")
                                        .executes(context -> LootTableFinder.prepareGlobalSearch("none_empty", context)
                        )))
                        .then(literal("summary").executes(SummaryBuilder::buildSummaryGlobal))
                        .then(literal("set_queue")
                                .then(argument("set_id", IntegerArgumentType.integer())
                                        .executes(Controller::setPlayerCoordinates)))
                        .then(literal("stop")
